...
const UserLikesSection = ({ userLikedNotes, icons }) => {
    return (
        <div>
            ...
            {userLikedNotes && userLikedNotes.length > 0 ? ( 
                <ul className="space-y-6">
                    {userLikedNotes.map((note) => ( 
                        <li 
                            key={note.likeId}
                            className="p-6 bg-rose-50 hover:bg-rose-100 transition-all duration-200 rounded-lg shadow-sm hover:shadow-md cursor-pointer border border-rose-100"
                        >
                            <p className="font-medium text-sm text-gray-800">{note.title}</p>
                            <p className="text-xs text-gray-600 mt-1">
                                {icons.UserCircle} {note.author} • {note.course}
                            </p>
