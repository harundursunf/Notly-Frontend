...
    // Not paylaşma formunun görünürlüğünü kontrol eden state
    const [isSharingNote, setIsSharingNote] = useState(false);

    // Aktif sekme state'i (paylaşımlar, beğenilenler, dersler)
    const [activeTab, setActiveTab] = useState('posts');

    // Yeni not formu verileri state'leri (Bunlar ShareNote'a taşınacak, burada sadece başlangıç değeri veriliyor)
...
    // Kullanıcı bilgileri (demo veriler)
    const user = {
        name: 'Harun Yılmaz',
...
    // Kullanıcının paylaştığı notlar (demo veriler)
...
    // Kullanıcının beğendiği notlar (demo veriler)
...
    // Topluluklar verisi (ShareNote'a gönderilecek)
...
        setActiveTab(tab);
        setIsSharingNote(false); // Sekme değiştiğinde not paylaşma formunu gizle
        // Form state'lerini burada temizlemeye gerek yok, ShareNote kendi içinde yönetecek
    };
...
    // "Not Paylaş" butonuna tıklandığında
    const handleShareNoteClick = () => {
        setIsSharingNote(true); // Not paylaşma formunu göster
        setActiveTab('posts'); // Paylaşımlar sekmesini aktif yap
    };
...
    // ShareNote bileşeninden gelen submit işlemi için callback fonksiyonu
    // ShareNote bileşeni bu fonksiyonu çağırdığında yeni notu postedNotes'a ekler
    const handleNewNoteShared = (newNote) => {
        setPostedNotes(prevNotes => [newNote, ...prevNotes]);
        // İlgili topluluğun not sayısını da burada güncelleyebiliriz
...
            );
        }
        setIsSharingNote(false); // Formu gizle
    };
...
                    >
                        {/* SVG yerine emoji ikon kullanıldı */}
...
                                {/* Koşullu Render: Not Listesi veya ShareNote Formu */}
                                {!isSharingNote ? (
                                    // Paylaşılan notlar varsa listeyi göster
...
                                        </ul>
                                    ) : (
                                        // Paylaşılan not yoksa mesaj göster
...
                                    
                                    <ShareNote
                                        setIsCreatingNote={setIsSharingNote} // isSharingNote state'ini güncelleyecek
                                        setNotes={setPostedNotes} // Paylaşılan notları güncelleyecek
                                        setCommunities={setCommunities} // Topluluk not sayılarını güncelleyecek
                                        communities={communities} // Topluluk verilerini gönderecek
                                    />
                                )}
                            </div>
                        )}

                        {/* Beğenilenler Sekmesi */}
                        {activeTab === 'liked' && (
                            <div>
